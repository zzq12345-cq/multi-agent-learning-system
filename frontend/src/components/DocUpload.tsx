    ...
    return (
      <div className="h-full flex items-center justify-center text-center px-6">
        <div>
          ...
          <p className="text-[10px] text-stone-500">选择学科后可上传学习资料</p>
        </div>
      </div>
    )
  }
    ...
    <div className="h-full flex flex-col bg-cream">
      ...
      <div className="p-4 border-b border-stone-200/50">
        <div
          ...
          className="border-2 border-dashed border-stone-200 rounded-xl p-4 text-center cursor-pointer hover:border-stone-400 hover:bg-surface transition-all"
        >
          <Upload className="w-5 h-5 text-stone-400 mx-auto mb-2" />
          <p className="text-[10px] text-stone-500 font-medium">
            {uploading ? '上传中...' : '点击上传学习资料'}
          </p>
          <p className="text-[8px] text-stone-400 mt-1">支持 PDF / Word / Markdown / TXT（≤10MB）</p>
        </div>
        <input
          ref={fileRef}
          type="file"
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {docs.length === 0 ? (
          <p className="text-[9px] text-stone-400">暂无文档，上传后 AI 将参考这些资料生成内容</p>
        ) : (
          <div className="space-y-1.5">
            {docs.map((doc) => (
              <div key={doc.filename} className="flex items-center gap-2 px-2.5 py-2 rounded-lg border border-stone-200/60 bg-surface">
                <FileText className="w-3.5 h-3.5 text-stone-400 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] text-stone-700 font-medium truncate">{doc.filename}</p>
                  <p className="text-[8px] text-stone-400">{(doc.size / 1024).toFixed(1)} KB</p>
                </div>
                <button
